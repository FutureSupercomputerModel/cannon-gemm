import CNN.layerFuserHelper as helper
import copy
import CNN.conv2gemm as conv2gemm

def should_tile_last_layer_and_fuse(cnn_layers, buffer_size, initial_tile_P, initial_tile_Q):
    ...
    inputSize = P_t*Q_t*cnn_layers[i][2]*cnn_layers[i][5]*cnn_layers[i][6] #PQRSC
...
def fuse_layer_recursive(cnn_layers, pooling_layers, buffer_size, optimal_strategies_to_layer):
    ...
    for initial_P in range(1, int(P_last)+1):
            initial_Q = initial_P
            i = len(cnn_layers)-1
            if(should_tile_last_layer_and_fuse(cnn_layers[0:i+1], buffer_size, initial_P, initial_Q)):
                fused_groups = []
                fused_layers = []
                fusing_params = []
                Q_t = initial_P
                ...
                while (IWBufferRemain-featureStorage) >= helper.get_weight_size(IWPrecision, cnn_layers[i]) and i>=0:
                    ...
                    fused_layers[0][0] = min(temp_W_t, fused_layers[0][0]+fused_layers[0][7]*2)
                    fused_layers[0][1] = min(temp_H_t, fused_layers[0][1]+fused_layers[0][8]*2)
                    ...
                    # print("fused_layers: "+str(fused_layers))
                    ...
                    if i>=0:
                        ...
                        if(optimal_strategies_to_layer[i] == []):   
                            upper_optimal_strategies = fuse_layer_recursive(cnn_layers[0:i+1], pooling_layers[0:i+1], buffer_size, optimal_strategies_to_layer)
                        else:
    ...
    print(f"call return {len(optimal_strategies)} strategies")
    return optimal_strategies
   

def fuse_layer_recursive_start(cnn_layers, pooling_layers, buffer_size):
    optimal_strategies_to_layer = []
    for i in range(len(cnn_layers)):
        optimal_strategies_to_layer.append([])
    optimal_strategies = fuse_layer_recursive(cnn_layers, pooling_layers, buffer_size, optimal_strategies_to_layer)
    for i in range(len(cnn_layers)):

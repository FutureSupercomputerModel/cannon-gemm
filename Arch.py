...
class Arch(Arch_base):
    ...
    def __init__(self, buffer_size:float, buffer_bw:float, alpha:float, mesh_bw:float, mesh_dim:float, child_arch:Arch_base) -> None:
    ...
    def cannon_gemm(self, m_in,k_in,n_in):
        ...
        T_compute = self.child_arch.get_gemm_latency(m_leaf, k_leaf, n_leaf)*self.mesh_dim
        ...
        return max(T_prep, T_compute, T_send, T_store)
    
    def get_gemm_latency(self, m,k,n):
        #report buffer usage
        ...
        T_tile=self.cannon_gemm(m_tile, k_tile, n_tile)
        return T_tile*iteration
...
hier_arch_leaf = Leaf(pe_arr_dim=200.0, pe_freq=1.0, buffer_size=20.0*1024*1024, buffer_bw=64.0)
hier_arch_0 = Arch(buffer_size=8*1024*1024*1024, buffer_bw=64.0*90.0*2, alpha=1.0, mesh_bw=64.0*1.0, mesh_dim=10.0, child_arch=hier_arch_leaf)
hier_arch_top = Arch(buffer_size=8*1024*1024*1024*1024, buffer_bw=64.0*90.0*2, alpha=1.0, mesh_bw=64.0*1.0, mesh_dim=10.0, child_arch=hier_arch_0)

def top_level_gemm(m,k,n, arch: Arch):
    print("=====================================")
    arch.print()
    print(f"top level problem: {m},{k},{n}")
    T_top=arch.get_gemm_latency(m, k, n)
